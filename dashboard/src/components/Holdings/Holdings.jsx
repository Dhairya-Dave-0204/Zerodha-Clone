...
import { VerticalGraph } from "../component_index";
...
const Holdings = () => {
    ...
    const fetchHoldings = async () => {
      try {
        const res = await axios.get(
          "http://localhost:8000/api/v1/holdings/all",
          {
  ...
  const stockLabels = holdings.map((stock) => stock.name);
  ...
  return (
    <>
          ...
          <table className="min-w-full border-collapse">
            ...
            <tbody>
              {holdings.map((stock, index) => {
                ...
                const isProfit =
                  curValue - stock.average * stock.quantity >= 0.0;
      ...
      <VerticalGraph data={holdingDataForGraph} />
    </>
  );
};
